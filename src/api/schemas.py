...
class EnrollFaceRequest(BaseModel):
    ...
    user_email: EmailStr | None = Field(None, description="User email address")
...
class FaceResponse(BaseModel):
    ...
    quality_score: float | None = Field(None, description="Face quality score")
    ...
    verified_at: datetime | None = Field(None, description="When photo was verified")
    verified_confidence: float | None = Field(None, description="Verification confidence score")
    verified_by_processor: str | None = Field(
        None, description="Recognition processor used (e.g., 'antelopev2', 'aws_rekognition')"
    )
...
class FaceMatchResponse(BaseModel):
    ...
    processor: str = Field(
        ...,
        description="Recognition processor used (e.g., 'antelopev2', 'antelopev2+aws', 'aws_rekognition')",
    )
...
class RecognizeMultipleFacesRequest(BaseModel):
    ...
    roi_enabled: bool = Field(False, description="Enable Region of Interest filtering")
    roi_x: float | None = Field(None, description="ROI X position (normalized 0-1)", ge=0.0, le=1.0)
    roi_y: float | None = Field(None, description="ROI Y position (normalized 0-1)", ge=0.0, le=1.0)
    roi_width: float | None = Field(None, description="ROI width (normalized 0-1)", ge=0.0, le=1.0)
    roi_height: float | None = Field(
        None, description="ROI height (normalized 0-1)", ge=0.0, le=1.0
    )
    min_overlap: float = Field(0.3, description="Minimum ROI overlap ratio (0-1)", ge=0.0, le=1.0)
...
class LivenessCheckRequest(BaseModel):
    ...
    threshold: float | None = Field(
        None,
        description="Liveness threshold (0.0-1.0, uses config default if not provided)",
        ge=0.0,
        le=1.0,
    )
...
class LivenessCheckResponse(BaseModel):
    ...
    spoofing_type: str = Field(
        ..., description="Type of spoofing detected (real/print/video/mask/unknown)"
    )
    ...
    details: dict | None = Field(None, description="Additional provider-specific details")
    execution_time: float = Field(..., description="Execution time in seconds")
